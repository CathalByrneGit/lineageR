...
`lin_run_complete()` automatically updates `last_updated_at` on the
transform node and all its output dataset nodes.
...
lin_plot(ctx, layout = "fr")             # change layout algorithm
```

`lin_plot()` uses **visNetwork** when available (returns an interactive
HTML widget), falls back to **ggraph** + **ggplot2** (static ggplot), and
finally to base **igraph** plotting.
...
To auto-wire the backing edge, add a lineage extension when defining the
object type:

```{r extension, eval = FALSE}
# In your ontologySpecR object type definition:
object_type("Airport",
  ...,
  extensions = list(
...
# Graft in the raw source and final output
...
| `lin_provenance()` | Data frame of upstream ancestors                     |
...
| `lin_run_complete()`  | Set `"success"`; update `last_updated_at` on outputs  |
| `lin_run_error()`     | Set `"error"` with message                            |
| `lin_run_history()`   | Recent runs for a transform                           |
...
lineageR persists everything in three tables:
...
Any DBI-compatible backend works (DuckDB, SQLite, PostgreSQL, …).  The
schema is initialised automatically by `lineage_context()` and is safe to
call multiple times.
